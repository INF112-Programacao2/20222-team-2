...
Tabuleiro::inicializarJogo()
{
  ...
  _tabuleiro[1][0] =
    new Cavalo(Cor::PRETO, { 1, 0 }, _knightBlackBmp, "Cavalo");
...
Tabuleiro::moverPeca(int x, int y, int x2, int y2)
{
  ...
  int qtmovimentospreto =
    0; // Variável para verificar quantos movimentos o preto fez
  int qtmovimentosbranco =
    0; // Variável para verificar quantos movimentos o branco fez
  int contplacarbrancas =
    0; // Variável para verificar quantos movimentos o preto fez
  int contplacarpretas =
    0; // Variável para verificar quantos movimentos o branco fez
  ...
  else if (_tabuleiro[x][y] != nullptr &&
           _tabuleiro[x][y]->getCor() == Cor::BRANCO &&
           _tabuleiro[x][y]->validarMovimento({ x1, y1 }) == true)
  {
    if (_tabuleiro[x2][y2] == nullptr)
    {
